...
PantallaSeleccionTablero::PantallaSeleccionTablero(GestorPantallas* gestor) : gestor(gestor) {
    botones.push_back({ "TABLERO BABY", -0.3f,  0.1f, 0.3f,  0.25f });
    botones.push_back({ "TABLERO GARDNER",         -0.3f, -0.2f, 0.3f, -0.05f });
}

void PantallaSeleccionTablero::dibuja() {
    BrochaPantallas::limpiar_pantalla(0.15f, 0.15f, 0.15f);
    BrochaPantallas::dibujar_texto("SELECCIONA TABLERO", -0.5f, 0.4f);
    for (auto& b : botones)
        b.dibujar();
}
